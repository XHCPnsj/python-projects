...
class Game:
    def __init__(self):
        self.tk=Tk()
        self.tk.title("stick guy escapes the matrix")
        ...
        self.bg=PhotoImage(file="giphy.gif")
        w=self.bg.width()
        h = self.bg.height()
        draw_backround=0
        for x in range(0,5):
...
class Coords:
    def __init__(self,x1=0, y1=0, x2=0, y2=0):
        ...
        self.x2=x2
        self.y2=y2
def within_x(co1,c02):
    if co1.x1 and co1.x1<co2.x2:
        return True
...
def within_y(co1,co2):
    if (co1.y1>co2.y1 and co1.y1<co2.y2) \
       ...
       or (co2.y1> co2.y1 and co2.y1< co1.y2) \
       or (co2.y2> co2.y1 and co2.y2< co1.y2):
        return True
    else:
        return false
...
def collided_bottom(co1,co2):
    if within_x(co1,co2):
        y_calc = c01.y2+y
        if y_calc >= co2.y1 and y-calc <= co2.y2:
            return True
    return False
    ...
    def animate(self):
        ...
        if self.x <0:
            if self.y !=0:
                ...
                self.game.canvas.itemconfig(self.image, \
                        image=self.images_left[self.currernt_image])
        elif self.x > 0:
                if self.y != 0:
                    self.game.canvas.itemconfig(self.image, \
                    ...
                    self.game.canvas.itemconfig(self.image, \
                            images=self.images_right[self.current_image])
    def coords(self):
    ...
    def move(self):
        ...
        for sprite in self.game.sprites:
            ...
            if top and self.y<0 and collided_top(co,sprite_co):
                self.y=-self.y
                top=False
            if bottom and self.y>0 and collided_top(self.y,\
                    co,sprite_co):
                ...
                if left and self.x <0 and collided_left(co,sprite_co):
                    self.x=0
                    left=False
                if right and self.x >0 and collideed_right(co,sprite_co):
                    self.x
                    right= False
            if falling and bottom and self.y == 0 \
               and co.y2 < self.game.anvas_height:
                self.y =4
